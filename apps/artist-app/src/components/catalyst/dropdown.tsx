...
export function DropdownItem({
  ...
  const classes = clsx(
    ...
    // Focus
    "data-[focus]:bg-accent data-[focus]:text-white",
    // Disabled state
    ...
    >
      {(Array.isArray(keys) ? keys : keys.split("")).map((char, index) => (
        <kbd
          key={index}
          className={clsx([
            "font-sans min-w-[2ch] text-center capitalize text-zinc-400 group-data-[focus]:text-white forced-colors:group-data-[focus]:text-[HighlightText]",
