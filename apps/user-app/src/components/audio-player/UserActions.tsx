...
function ClaimButton({ onClick, disabled = true }: ClaimButtonProps) {
  ...
  return (
    <Button
      className={cn(
        "gap-1.5 w-[185px] md:w-auto h-[48px] md:h-auto rounded-[120px] px-2.5 py-2 font-figtree text-xs font-semibold tracking-[-0.04em] transition-none disabled:opacity-100",
        disabled
...
function DiscoverArtistButton({ onClick }: { onClick: () => void }) {
  return (
    <Button
      className="rounded-[120px] w-[185px] md:w-auto h-[48px] md:h-auto bg-white px-4 py-2 font-figtree text-xs font-semibold tracking-[-0.02em] text-[#0F0F0F] hover:bg-white/90"
      onClick={onClick}
    >
